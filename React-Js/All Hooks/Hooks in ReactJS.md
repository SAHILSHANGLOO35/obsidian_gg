...
For example - In the following example, there are two functions which are returning the same statement, but when we check _**fn1 === fn2**_, it's false - because both functions are created on different memory locations, so these are not same functions and hence giving false.
...
Therefore, We can solve this by using ==useCallback Hook== which caches the function in memory and uses that only preventing re-rendering of the ==<Header /> component== as shown below:
...
export const ContextProvider = ({children}: {children: React.ReactNode}) => {
    const phone = "+91 6006";
    return <AppContext.Provider value={phone}>{children}</AppContext.Provider>;
};
```
...
The image shows the prop drilling from top level APP > ["Footer", ["Profile" > "Contact"]]

![[Pasted image 20260108011436.png]]
